m4bchsplt() {
    ...
    # Find all M4B files in current directory, sorted by name
    mapfile -t the_files < <(find . -maxdepth 1 -name "*.m4b" -type f | sort -V)
    
    if [[ ${#the_files[@]} -eq 0 ]]; then
        echo "Error: No M4B files found in current directory"
        return 1
    fi

    echo "Found ${#the_files[@]} M4B file(s)"
    ...
    for input_file in "${the_files[@]}"; do
